...
function out = forces_moments(x, delta, wind, P)
    ...
    R_b_v = [cos(psi)*cos(theta) -sin(psi)*cos(phi)+cos(psi)*sin(theta)*sin(phi) sin(psi)*sin(phi)+cos(psi)*sin(theta)*cos(phi);
        sin(psi)*cos(theta) cos(psi)*cos(phi)+sin(psi)*sin(theta)*sin(phi) -cos(psi)*sin(phi)+sin(psi)*sin(theta)*cos(phi);
        -sin(theta) cos(theta)*sin(phi) cos(theta)*cos(phi)]; %L2p17
    ...
    Va = sqrt(V_a_b(1)*V_a_b(1)+V_a_b(2)*V_a_b(2)+V_a_b(3)*V_a_b(3));%Va=norm(V_a_b)
    ...
    gForce =  [-P.mass*P.gravity*sin(theta);
        P.mass*P.gravity*cos(theta)*sin(phi);
        P.mass*P.gravity*cos(theta)*cos(phi)]; %L4 p13
    ...
    aForce = 1/2*P.rho*Va^(2)*P.S_wing*[C_X+C_X_q*(P.c/(2*Va))*q+C_X_delta_e*delta_e;
        ...
        C_Z+C_Z_q*(P.c/(2*Va))*q+C_Z_delta_e*delta_e]; %p17
    ...
    Torque = 1/2*P.rho*Va^(2)*P.S_wing*[P.b*(P.C_ell_0+P.C_ell_beta*beta+P.C_ell_p*(P.b/(2*Va))*p+P.C_ell_r*(P.b/(2*Va))*r+P.C_ell_delta_a*delta_a+P.C_ell_delta_r*delta_r);
        ...
        P.b*(P.C_n_0+P.C_n_beta*beta+P.C_n_p*(P.b/(2*Va))*p+P.C_n_r*(P.b/(2*Va))*r+P.C_n_delta_a*delta_a+P.C_n_delta_r*delta_r)]+[-P.k_T_P*(P.k_Omega*delta_t)^(2);
        0;
        0];
